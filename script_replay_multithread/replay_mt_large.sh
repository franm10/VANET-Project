...
SIM_TIME=290
MAX_ERRORS_PER_GROUP=25   # Errori tollerati per ogni gruppo prima di fermare tutto

SCENARIOS=(
    "S1_NoAttack_Large_noattack"
    ...
    "S3_RSA_Large_60pct"
    "S4_ECDSA_Large_8pct"
    "S4_ECDSA_Large_15pct"
    "S4_ECDSA_Large_30pct"
    "S4_ECDSA_Large_40pct"
    "S4_ECDSA_Large_50pct"
    "S4_ECDSA_Large_60pct"
    "S5_RSA_NoAttack_Large"
    "S6_ECDSA_NoAttack_Large"
)
...
run_one() {
    ...
    opp_run "${OPP_ARGS[@]}" -c "$scenario" -r "$r" omnetpp_large.ini \
...
if [ -f "$RESULTS_DIR/.sim_error" ]; then
    ...
    METRICS_FILE="$RESULTS_DIR/metriche_replay_large.txt"

    echo "================================================================" > "$METRICS_FILE"
    echo "METRICHE COMPLETE — Replay Attack — Large Topology" >> "$METRICS_FILE"
    ...
    mac_s1=$(get_mac_avg "S1_NoAttack_Large_noattack")
    mac_s5=$(get_mac_avg "S5_RSA_NoAttack_Large")
    mac_s6=$(get_mac_avg "S6_ECDSA_NoAttack_Large")
