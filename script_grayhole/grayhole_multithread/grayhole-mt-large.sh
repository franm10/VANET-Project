...
RESULTS_DIR="$WORK_DIR/results_parallel_${TIMESTAMP}_large"
...
cp omnet_large.ini "$RESULTS_DIR/omnet_large.ini"
...
run_one() {
    ...
    opp_run "${OPP_ARGS[@]}" -c "$scenario" -r "$r" omnet_large.ini \
...
if [ -f "$RESULTS_DIR/.sim_error" ]; then
    ...
    METRICS_FILE="$RESULTS_DIR/metriche_grayhole_large.txt"

    echo "================================================================" > "$METRICS_FILE"
    echo "METRICHE COMPLETE — Grayhole Attack — Large Topology" >> "$METRICS_FILE"
